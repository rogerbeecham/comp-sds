...
Run the code block below to download and process the **crashes**, *vehicles** and **casualties** data. These are large datasets so this may take a little time to execute.
...
You may well have had problems due to the fact that we are working with a large dataset, so I have made `ped_veh` available on a personal repository. Run the block below ti download.
