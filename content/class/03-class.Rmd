...
The template file lists the required packages -- `tidyverse`, `sf` and also the [`parlitools`](https://cran.r-project.org/web/packages/parlitools/index.html). Installing `parlitools` brings down the 2019 UK General Election dataset, along with other constituency-level datasets. Loading it with `library(parlitools)` makes these data available to your R session.
...
{{% callout note %}}
Read this design story by [Lunzner and McNamara, 2020](http://tinlizzie.org/histograms/) for an excellent discussion of the analysis and design considerations when working with histograms. There are of course other *geoms* for summarising over 1D distributions: `geom_boxplot()`, `geom_dotplot()`, `geom_violin()`.
{{% /callout %}}
...
Previously we calculated overall vote share by Political Party. We could continue the exploration of votes by region by re-using this code to generate plots displaying quantities but also comparing region, using marks and encoding channels that are suitable for magnitudes.
...
names(party_colours) <- levels(temp_party_shares %>% pull(party))
```
